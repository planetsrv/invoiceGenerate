...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'save_company') {
            ...
            // Kolom contact_name digunakan untuk menyimpan nomor customer service (CS).
            $companyCs = trim((string)($_POST['contact_name'] ?? ''));
            ...
            if (strlen($companyName) > 150 || strlen($companyCs) > 100
            ...
            if (is_array($logo) && (int)($logo['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
                ...
                $logoWidth = (int)($imageInfo[0] ?? 0);
                $logoHeight = (int)($imageInfo[1] ?? 0);
                if ($logoWidth > 980 || $logoHeight > 980) {
                    throw new RuntimeException('Dimensi logo maksimal 980 × 980 piksel. Rekomendasi ukuran 800 × 260 piksel.');
                }
                $allowedLogoTypes = [
                    'image/png' => 'png',
                    'image/jpeg' => 'jpg',
        ...
        // Normalisasi izin hanya dari ID billing yang benar-benar tersedia.
        // Akun staf wajib mempunyai minimal satu billing agar tidak tersimpan
        // sebagai akun aktif yang selalu mendapatkan hasil data kosong.
        $submittedBillingIds = is_array($_POST['billing_ids'] ?? null)
            ? array_map('intval', $_POST['billing_ids'])
            : [];
        $billingIds = array_values(array_intersect(
            array_map('intval', array_keys($billingRows)),
            array_values(array_unique($submittedBillingIds))
        ));
        if (!$billingIds) {
            throw new RuntimeException('Pilih minimal satu akses billing untuk user.');
        }
        ...
        foreach ($billingIds as $selectedBillingId) {
            $billingId = (int)$selectedBillingId;
            $stmt->execute();
        }
        $stmt->close();

        // Jangan sahkan transaksi apabila sebagian izin gagal tercatat.
        $stmt = $db->prepare(
            "SELECT COUNT(*) AS total FROM user_billing_access WHERE user_id = ?"
        );
        $stmt->bind_param('i', $userId);
        $stmt->execute();
        $savedAccessCount = (int)($stmt->get_result()->fetch_assoc()['total'] ?? 0);
        $stmt->close();
        if ($savedAccessCount !== count($billingIds)) {
            throw new RuntimeException('Akses billing gagal disimpan lengkap. Silakan coba kembali.');
        }

        $selectedBillingNames = array_map(
            static fn(int $id): string => (string)$billingRows[$id],
            $billingIds
        );
        $successMessage .= ' Akses: '.implode(', ', $selectedBillingNames).'.';
...
while ($row = $result->fetch_assoc()) {
    $row['billing_ids'] = [];
    $row['customer_prefix'] = null;
    $managementUsers['staff-'.(int)$row['id']] = $row;
}
$stmt->close();

$customerUsers = [];
$stmt = $db->prepare("SELECT id, username, password, full_name, customer_prefix, is_active, created_at
    ...
    <section class="card panel company-settings-card" id="company-profile">
        <div class="card-body p-4">
            <div class="management-section-heading">
                ...
                <div class="company-logo-field">
                    <label
                        ...
                        class="company-logo-preview <?= $hasCompanyLogo ? '' : 'is-empty' ?>"
                    ...
                    >
                        ...
                        <?php endif; ?>
                        <span class="company-logo-action">Upload Logo</span>
                    </label>
                    <span class="company-logo-help" id="companyLogoHelp">Rekomendasi 800 × 260 px · Maks. 980 × 980 px</span>
                </div>
            </div>
            ...
            <form method="post" enctype="multipart/form-data" class="company-settings-form">
                ...
                <!-- Input disembunyikan karena area pratinjau di atas menjadi tombol pilih logo. -->
                <input id="companyLogo" type="file" name="company_logo" class="visually-hidden" accept="image/png,image/jpeg,image/webp">
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <label for="companyContact" class="form-label">Nomor CS</label>
                        <input id="companyContact" type="tel" inputmode="tel" name="contact_name" class="form-control" maxlength="100" placeholder="Contoh: +6232220000" value="<?= htmlspecialchars($companySettings['contact_name']) ?>">
                    </div>
                    <div class="col-md-4">
                        <label for="companyPhone" class="form-label">Nomor telepon</label>
                        <input id="companyPhone" type="tel" inputmode="tel" name="phone" class="form-control" maxlength="50" placeholder="Contoh: +6231110000" value="<?= htmlspecialchars($companySettings['phone']) ?>">
                    </div>
                    <div class="col-md-4">
                                    ...
                                    <div class="file-entry-meta account-entry-meta">
                                        <span>
                                            ...
                                            <?php if ($user['role'] === 'admin'): ?>
                                                Semua billing
                                            <?php elseif ($user['role'] === 'customer'): ?>
                                                Customer area &middot; <?= htmlspecialchars($user['customer_prefix'] ?: 'Tanpa pelanggan') ?>
                                            <?php else: ?>
                                                <?php if ($assignedBillingNames): ?>
                                                    <?= htmlspecialchars(implode(', ', $assignedBillingNames)) ?>
                                                <?php else: ?>
                                                    <strong class="text-danger">Belum ada akses billing</strong>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                        </span>
...
<script>
    // Tampilkan logo yang baru dipilih tanpa menunggu formulir disimpan.
    const companyLogoInput = document.getElementById('companyLogo');
    const companyLogoPreview = document.getElementById('companyLogoPreview');
    const companyLogoHelp = document.getElementById('companyLogoHelp');
    const companyLogoHelpDefault = 'Rekomendasi 800 × 260 px · Maks. 980 × 980 px';

    companyLogoInput?.addEventListener('change', () => {
        const selectedLogo = companyLogoInput.files?.[0];
        ...
        previewImage.alt = 'Pratinjau logo perusahaan';
        previewImage.addEventListener('load', () => {
            if (previewImage.naturalWidth > 980 || previewImage.naturalHeight > 980) {
                URL.revokeObjectURL(previewUrl);
                companyLogoInput.value = '';
                companyLogoHelp.textContent = `Logo ditolak: ${previewImage.naturalWidth} × ${previewImage.naturalHeight} px melebihi batas 980 × 980 px.`;
                companyLogoHelp.classList.add('is-error');
                return;
            }

            uploadLabel.className = 'company-logo-action';
            uploadLabel.textContent = 'Upload Logo';
            companyLogoPreview.replaceChildren(previewImage, uploadLabel);
            companyLogoPreview.classList.remove('is-empty');
            companyLogoHelp.textContent = companyLogoHelpDefault;
            companyLogoHelp.classList.remove('is-error');
            URL.revokeObjectURL(previewUrl);
        }, { once: true });
        previewImage.addEventListener('error', () => {
            URL.revokeObjectURL(previewUrl);
            companyLogoInput.value = '';
            companyLogoHelp.textContent = 'Logo tidak dapat dibaca. Pilih file PNG, JPG, atau WEBP.';
            companyLogoHelp.classList.add('is-error');
        }, { once: true });
